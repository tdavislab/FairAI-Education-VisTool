...
def load_preproc_data_bank():
    def custom_preprocessing(df):
        # Group age by decade
        df['age'] = df['age'].apply(lambda x: x//10*10)
    
        def group_edu(x):
            if x in ['basic.9y', 'basic.6y', 'basic.4y', 'high.school', 'illiterate']:
                return 'basic'
            else:
                return 'educational'
        ...
        def group_job(x):
            if x in ['admin.','blue-collar', 'management', 'entrepreneur', 'technician', 'entrepreneur', 'services', 'housemaid']:
                return 'manage'
            else:
                return x
        ...
        # Limit education range
        df['education'] = df['education'].apply(lambda x: group_edu(x))
        df['age'] = df['age'].apply(lambda x: age_cut(x))
        df['duration'] = df['duration'].apply(lambda x: group_duration(x))
        df['job'] = df['job'].apply(lambda x: group_job(x))

        return df

    XD_features = ['age', 'job', 'marital', 'education', 'default', 'loan', 'duration']
    D_features = ['marital']
    Y_features = ['y']
    X_features = list(set(XD_features)-set(D_features))
    categorical_features = ['age', 'job', 'education', 'default', 
    'loan', 'duration']

    # privileged classes
    privileged_classes= [['unmarried']]
    ...
    return BankDataset(
        ...
        instance_weights_name=None,
        categorical_features=categorical_features,
        features_to_drop=['campaign', 'pdays', 'month', 'day_of_week',  'default', 'loan', 'housing', 'contact', 'previous', 'poutcome', 'emp.var.rate', 'cons.price.idx', 'cons.conf.idx', 'euribor3m', 'nr.employed'],
